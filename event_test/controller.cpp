...
int Controller::rdma_read_once(std::string msg, int invoker_id) {
...
int Controller::rdma_write_while(int thread_id) {
	struct ibv_qp *qp = res.qp[thread_id];
	struct ibv_cq *cq = res.cq[thread_id];
	struct ibv_wc *wc = nullptr;
	uint32_t write_rkey = res.cache_rkey[thread_id];
	uint64_t write_raddr = res.cache_raddr[thread_id];
	uint32_t lkey = res.cache_mr->lkey;
...
int Controller::run() {
	std::thread read_thread(&Controller::rdma_write_while, this, 0);
	read_thread.join();
